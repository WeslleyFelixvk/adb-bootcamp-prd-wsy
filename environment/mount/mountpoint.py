...
dbutils.fs.mount(
    source = "wasbs://raw@dlsbootcamptestewsy.blob.core.windows.net"
    ,mount_point = "/mnt/raw/"
    ,extra_configs = {"fs.azure.account.key.dlsbootcamptestewsy.blob.core.windows.net" :dbutils.secrets.get(scope = "scopo-kv-bootcamp-teste-wsy", key = "secret-dlsbootcamptestewsy")}
)
...
config =  {"fs.azure.account.key.dlsbootcamptestewsy.blob.core.windows.net" :dbutils.secrets.get(scope = "scopo-kv-bootcamp-teste-wsy", key = "secret-dlsbootcamptestewsy")}
...
def mount_diretorio_lake(lst_diretorios):
    try:        
        for diretorio in lst_diretorios:
            dbutils.fs.mount(
                source = f"wasbs://{diretorio}@dlsbootcamptestewsy.blob.core.windows.net"
